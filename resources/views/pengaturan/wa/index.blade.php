<x-app-layout>
                                        ...
                                        <tbody class="divide-y divide-gray-200">
                                            ...
                                            <tr>
                                                ...
                                                <td class="p-3 whitespace-nowrap text-sm font-medium text-gray-800 border">
                                                    {{strtoupper($d->untuk)}}</td>
                                                <td class="p-3 whitespace-nowrap text-sm text-gray-800 border">{{$d->group_id ?? '---------'}}</td>
                                                <td class="p-3 whitespace-nowrap text-center text-sm font-medium border">
                                                    <div class="inline-flex">
                                                        ...
                                                        <x-secondary-button class="m-2 bg-yellow-300 hover:bg-yellow-600" type="button" @click="openEditModal('{{ $d->untuk }}', '{{ $d->group_id }}', '{{ $d->id }}', '{{$d->nama_group}}')">
    ...
    <script>
        // open modal edit lalu tambahkan pilihan option dari route pengaturan.group-wa.get-group-wa
        function openEditModal(edit_untuk, edit_group_id, edit_id, edit_nama_group) {
            // Show loader
            document.getElementById('loader').classList.remove('hidden');
            // Dispatch event to open modal
            const event = new CustomEvent('open-modal', {
                detail: { id: 'editModal', edit_untuk: edit_untuk, edit_group_id: edit_group_id, edit_nama_group: edit_nama_group, edit_id: edit_id }
            });

            window.dispatchEvent(event);

            // Fetch data and add options to select
            fetch('/pengaturan/group-wa/get-group-wa')
                .then(response => response.json())
                .then(data => {
                    const select = window.HSSelect.getInstance('#edit_nama_group');
                    const options = data.groups.map(group => ({
                        title: group.name,
                        val: group.id,
                        selected: group.id == edit_nama_group
                    }));
